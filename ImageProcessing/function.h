...
void  FastFourierTran(CplexNum * pTd, CplexNum * pFd, int power)
{
	...
	dotCount = 1 << power;// 计算付立叶变换点数	dotCount=1*2^power  dotCount 就是N=2^n	
	pWn = new CplexNum[sizeof(CplexNum)*dotCount / 2];// 分配运算所需存储器 就是Wn
...
* 函数功能: 用来对图像进行离散余弦变换
************************************************************************/
double *allDctpFd = NULL;
double *allDctpTd = NULL;
BOOL  CosTran(LPSTR lpSrcStartBits, long lWidth, long lHeight, long lLineBytes)
{
	unsigned char*	lpSrcUnChr;	//指向像素的指针
	long i;                 //行循环变量
	long j;                 //列循环变量 			
	long	wid = 1, hei = 1;// 进行离散余弦变换的宽度和高度，初始化为1
	double	dTemp;// 中间变量	
	int		widpor = 0, heiPor = 0;//2的幂数
	...
	allDctpFd = new double[wid*hei];
	allDctpTd = new double[wid*hei];
	...
	memcpy(allDctpFd, pFd, sizeof(double)*wid * hei);
	memcpy(allDctpTd, pTd, sizeof(double)*wid * hei);

	delete pTd;// 释放内存
	delete pFd;
	return TRUE;
}
BOOL  InverseCosTran(LPSTR lpSrcStartBits, long lWidth, long lHeight, long lLineBytes)
{
	unsigned char*	lpSrcUnChr;	//指向像素的指针
	long i;                 //行循环变量
	long j;                 //列循环变量 			
	...
	for (i = 0; i < hei; i++)
	{
		InverseDisFCosTran(&allDctpTd[wid * i], &allDctpFd[wid * i], widpor);// 对y方向进行离散余弦变换
	}
	for (j = 0; j < wid; j++)// 保存计算结果
	{
		for (i = 0; i < hei; i++)
		{
			allDctpFd[j * hei + i] = allDctpTd[j + wid * i];
		}
	}
	for (j = 0; j < wid; j++)
	{
		InverseDisFCosTran(&allDctpTd[j * hei], &allDctpFd[j * hei], heiPor);// 对x方向进行离散余弦变换
	}
	for (j = 0; j < wid; j++)// 行
	{
		for (i = 0; i < hei; i++)// 列
		{
			dTemp = fabs(allDctpTd[i*wid + j]);// 计算频谱
...
{
	long	dotCount;// 沃尔什-哈达玛变换点数 即MxN维矩阵	
	int		i, j, k;// 循环变量	
	int		bfsize, p;// 中间变量	
	...
	{
		for (j = 0; j < 1 << k; j++)//j==1  j<1
		{
			bfsize = 1 << (power - k);//8
			for (i = 0; i < bfsize / 2; i++)//4
			{
				p = j * bfsize;//0
				temReg2[i + p] = temReg1[i + p] + temReg1[i + p + bfsize / 2];//f0+f4=f(1)0
				temReg2[i + p + bfsize / 2] = temReg1[i + p] - temReg1[i + p + bfsize / 2];//f(0)-f(4)=f(1)4
			}
		}
...
BOOL  Walsh_HarTran(LPSTR lpSrcStartBits, long lWidth, long lHeight, long lLineBytes)
{
	...
	for (i = 0; i < hei; i++)// 行
	{
		...
		for (j = 0; j < wid; j++)// 列
		{
			// 指向DIB第i行，第j个像素的指针
			lpSrcUnChr = (unsigned char*)lpSrcStartBits + lLineBytes * (lHeight - 1 - i) + j;
			pTd[j + i * wid] = *(lpSrcUnChr);// 给时域赋值 横向赋值
		}
	}
	for (i = 0; i < hei; i++)
	{
